...
xyplot(
  LaktNeuinf_rel + LaktHeilung_rel + Eutergesund_rel + Unheilbar_rel ~ as.Date(Monat),
  data = ds_monatlich,
  type = c("l", "g"),
...
histogram(c(ds1$LaktTag[which(ds1$VorZZ <= settings$GW[1] & ds1$ZZ > settings$GW[1])],
            ds1$LaktTag[which(ds1$Punkt == "F" & ds1$ZZ > settings$GW[1])]),   # alle Neuinfektionen (incl. ueber Trockenstehphase und Färsenmastitis)
          type = "percent",
...
\section*{Stoffwechseluebersicht}
