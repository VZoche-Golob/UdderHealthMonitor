...
#' @out_dir Optional character of length 1. Full path to the directory where the report
#'  file should be produced. If not specified the directory of \code{PCstart} is used.
#' @out_window Logical of length 1. Should the report be shown in the standard plot display?
#'  (Default is \code{FALSE})
#' @return_results Logical of length 1. Should the data shown in the report be returned
...
#'  directory. If \code{return_results} is \code{TRUE}, the data is also returned
#'  as data.frame. \code{NULL} invisible otherwise.
#'
...
#' @note This function does not require LaTeX.
#'
#' @importFrom magrittr '%>%'
#'
#' @export
...
IndicatorSheet <- function(
  ...
  for (p in c("Lactation", "DryPeriod")) {
