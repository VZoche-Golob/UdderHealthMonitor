#' Reads ADIS entity 880028.
#'
#' Used inside prepare_PCstart.
#'
#' @param header_list A list as returned by \link{read_adis} containing the ADIS entities of one single header.
...
#' Prepares the data of the 'PCstart' ADIS files from VIT for udder health monitoring.
#'
#' Used inside \link{monitor_SCCUdderHealth}.
#'
#' @param file Character of length 1. Full path to the PCStart ADIS file to be read, expected in UNIX-style ('/').
...
prepare_PCstart <- function(file, ...) {
  ...
  if(nrow(daten2) != nrow(unique(daten2[,c("KuhID","Monat")]))) {
    warning(paste(sum(duplicated(daten2[,c("KuhID","Monat")])),"Mehrfacheintraege pro Kuh und Monat entfernt (von",nrow(daten2),"Eintr\U00E4gen)."))
  ...
  if(any(daten2$ZZ==0,na.rm=TRUE)) {
    warning(paste(sum(daten2$ZZ==0,na.rm=TRUE), "Zellzahlwerte von 0 auf NA gesetzt (von",nrow(daten2),"Eintr\U00E4gen)."))
    daten2$ZZ[daten2$ZZ==0] <- NA
  }
  if(any(daten2$Fp==0,na.rm=TRUE)) {
    warning(paste(sum(daten2$Fp==0,na.rm=TRUE), "Fettprozente von 0 auf NA gesetzt (von",nrow(daten2),"Eintr\U00E4gen)."))
    daten2$Fp[daten2$Fp==0] <- NA
  }
  if(any(daten2$Ep==0,na.rm=TRUE)) {
    warning(paste(sum(daten2$Ep==0,na.rm=TRUE), "Eiweissprozente von 0 auf NA gesetzt (von",nrow(daten2),"Eintr\U00E4gen)."))
    daten2$Ep[daten2$Ep==0] <- NA
  }
  if(any(daten2$Urea==0,na.rm=TRUE)) {
    warning(paste(sum(daten2$Urea==0,na.rm=TRUE), "Harnstoffwerte von 0 auf NA gesetzt (von",nrow(daten2),"Eintr\U00E4gen)."))
...
#' Used inside \link{monitor_SCCUdderHealth}.
#'
#' @param inddata A dataframe provided as \code{$einzeltiere} by \link{prepare_PCstart}.
...
prepare_SCCdata <- function(inddata, Months = c(1:12)) {
    ...
    for(l in unique(ds_kuh$LaktNr)) {
      ...
      if(l!=min(ds_kuh$LaktNr)) {
        if(any(!is.na(ds_kuh$ZZ[ds_kuh$LaktNr==(l-1)]))) {
          ds_lakt$VorZZ[1] <- ifelse(ds_lakt$Punkt[1]=="T",ds_kuh$ZZ[ds_kuh$LaktNr==(l-1)&ds_kuh$LaktTag==max(ds_kuh$LaktTag[ds_kuh$LaktNr==(l-1)&!is.na(ds_kuh$ZZ)])],NA)   # letzte verfuegbare Zellzahl in der vorherigen Laktation
        }
      }
      ...
      if(l!=min(ds_kuh$LaktNr)) {
        if(any(!is.na(ds_kuh$ZZ[ds_kuh$LaktNr==(l-1)]))) {
          ds_lakt$VorVorZZ[1] <- ifelse(ds_lakt$Punkt[1]=="T",ds_kuh$ZZ[ds_kuh$LaktNr==(l-1)&ds_kuh$LaktTag==sort(ds_kuh$LaktTag[ds_kuh$LaktNr==(l-1)&!is.na(ds_kuh$ZZ)],decreasing=TRUE)[2]],NA)   # vorletzte verfuegbare Zellzahl in der vorherigen Laktation
        }
      }
  ...
  ds1 <- subset(ds1, ds1$Monat%in%sort(unique(ds1$Monat),decreasing=TRUE)[Months])   # auf letzte 12 Monate begrenzen
...
#' Calculates the most important SCC udder health indicators for a given dataframe.
#'
#' Used inside \link{monitor_SCCUdderHealth}.
#'
#' @param daten A dataframe as returned by \link{prepare_SCCdata}.
#' @param kennzahl A character string, one of
...
kennzahlen_bestimmen <- function(daten,kennzahl,gw,relativ=TRUE) {
  if(relativ) {
    ausgabe <- switch(kennzahl,
                      ErstMLPKuh=round(sum(daten$Punkt=="T")/(sum(daten$Punkt=="T")+sum(daten$Punkt=="F")),1),
                      ErstMLPFaerse=round(sum(daten$Punkt=="F")/(sum(daten$Punkt=="T")+sum(daten$Punkt=="F")),1),
                      FaersenMast=round(sum(daten$Punkt=="F"&daten$ZZ>gw,na.rm=TRUE)*100/sum(daten$Punkt=="F"),1),
                      TSNeuinf=round(sum(daten$Punkt=="T"&daten$ZZ>gw&daten$VorZZ<=gw,na.rm=TRUE)*100/sum(daten$Punkt=="T"&daten$VorZZ<=gw,na.rm=TRUE),1),
                      TSHeilung=round(sum(daten$Punkt=="T"&daten$ZZ<=gw&daten$VorZZ>gw,na.rm=TRUE)*100/sum(daten$Punkt=="T"&daten$VorZZ>gw,na.rm=TRUE),1),
                      LaktNeuinf=round(sum(daten$Punkt=="L"&daten$ZZ>gw&daten$VorZZ<=gw,na.rm=TRUE)*100/sum(daten$Punkt=="L"&!is.na(daten$ZZ)&daten$VorZZ<=gw,na.rm=TRUE),1),
                      LaktHeilung=round(sum(daten$Punkt=="L"&daten$ZZ<=gw&daten$VorZZ>gw,na.rm=TRUE)*100/sum(daten$Punkt=="L"&!is.na(daten$ZZ)&daten$VorZZ>gw,na.rm=TRUE),1),
                      LaktChronisch=round(sum(daten$Punkt=="L"&daten$ZZ>gw&daten$VorZZ>gw,na.rm=TRUE)*100/sum(!is.na(daten$ZZ)&!is.na(daten$VorZZ)),1),
                      Eutergesund=round(sum(daten$ZZ<=gw,na.rm=TRUE)*100/sum(!is.na(daten$ZZ)),1),
                      Unheilbar=round(sum(daten$ZZ>gw&daten$VorZZ>gw&daten$VorVorZZ>gw,na.rm=TRUE)*100/sum(!is.na(daten$ZZ)),1),
                      "Welche Kennzahl?")
  } else {
    ausgabe <- switch(kennzahl,
...
einzeltierliste_erstellen <- function(mlpdaten,kuehe,monate=3) {
  if(length(kuehe)==0) {
    stop("Keine K\U00FChe ausgew\U00E4hlt!")
  }
  mlpdaten <- subset(mlpdaten, KuhID%in%kuehe)[,c("Monat","KuhID","Stallnummer","Name","Kalbedatum","LaktNr","ZZ")]
